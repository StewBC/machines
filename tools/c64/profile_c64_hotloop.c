...
static bool load_default_roms(c64_rom_set *roms, char *error, size_t error_size) {
    ...
    return c64_rom_load_combined_64c(roms, MACHINES_ROMS_DIR "/system.rom", error, error_size) &&
        c64_rom_load_character(roms, MACHINES_ROMS_DIR "/character.rom", error, error_size);
}
...
int main(int argc, char **argv) {
    ...
    if (load_1541) {
        if (c1541_load_rom(&machine.drive8, MACHINES_ROMS_DIR "/1541.rom") == 0) {
            fprintf(stderr, "failed to load %s/1541.rom into drive8\n", MACHINES_ROMS_DIR);
            return 1;
        }
        if (drive9_rom) {
            if (c1541_load_rom(&machine.drive9, MACHINES_ROMS_DIR "/1541.rom") == 0) {
                fprintf(stderr, "failed to load %s/1541.rom into drive9\n", MACHINES_ROMS_DIR);
                return 1;
            }
        }
