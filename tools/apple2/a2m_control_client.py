#!/usr/bin/env python3
"""A2M/16 control-port client for a2m.
...
GOTCHAS (Apple A2M/16):
  * Identity: hello -> name=a2m protocol=A2M/16
  * printer-flush: fire-and-forget force-flush of a dirty ImageWriter page when
    SSC is installed (no-op if clean/blank/absent). Enable/dir/format stay
    Configure/INI/CLI — no set-printer/get-printer in v1.
  * Inspector: get-state includes mode=live|inspector focus_cycle= start= start_arg1=.
    leave-inspector restores live NOW (no auto-resume). land-inspector cycle=N
    (quantized) and land-inspector-exact cycle=N land the Record tape; from live
    they imply enter. While Inspecting, run/step-* are sealed execute clamped to
    live; pokes/media/reset/printer-flush fail with error read-only-inspector.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="a2m control client (A2M/16)")
