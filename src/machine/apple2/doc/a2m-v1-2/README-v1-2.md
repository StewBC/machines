...
## User Manual
The user manual is in markdown, but written in a format that can be turned into a PDF.  The PDF is in the releases, but the markdown also available for viewing in a browser, with some extra mark-up, [here](manual.md)  
...
* Mockingboard  
  - The hardware files via6522, ay38910 and mboard were generated by Codex in an experiment to learn more about "agentic coding".  The result works, even if it doesn't sound nearly as good as say AppleWin.  I am likely not going to try and improve it.
  - When the machine reboots, the MB isn't fully reset.  It needs a OA+CTRL+RESET (ALT+F2 or OPTION+F2) to fully reset the MB.  If not, it may well not be detected a second time.
* General  
...
* tomcw for mb-audit.  That's a program to tell emulator writers if their Mockingboard meets spec (https://github.com/tomcw/mb-audit)
