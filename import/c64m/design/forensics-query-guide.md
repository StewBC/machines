...
This is the detailed design for the Forensics **query line** (ships in
[`forensics-ui.md`](forensics-ui.md) **PR 4**). Normative product rules are also
restated in the parent doc so implementers cannot miss them.

---

## Overview

The Forensics query line is a tiny **verb-first** language. Tab is a
**left-to-right walker**: it either unique-completes expected terminals or
prints **that hole's** ASCII help on the status strip. When the caret is at the
end of the line, Tab also **unique-expands every token** on the line (not only
the last).

Control-port `history-find` / `history-read` are unchanged. Bare `key=value` on
the wire remains valid. This is the Forensics query line only. Dropping
implicit FIND (`address=$D020` with no `find`) on the UI line is intentional.

**Reuse:** Lift the a2m `forensics_view_autocomplete` / walker implementation
where it fits; point find keys/access at c64m’s shared
`runtime_history_query_parse` tables (parent PR 1).

---

## Why not last-token find-key Tab alone

A Tab that only completes the last whitespace token against find option keys
does not know verbs, does not know `read`'s `before=` / `after=` / `epoch=`,
and does not teach value syntax. Empty Tab would print a find-key hint;
`read`<Tab> would look like a failed find-key prefix. That approach is
rejected for c64m — ship the walker in PR 4.

**Caret at end after rewrite remains required:** any buffer rewrite still has
to set Nuklear `edit.cursor` explicitly (parent Key Decisions).
...
3. **Unique or teach.** At any hole: prefix of exactly one expected terminal -> rewrite to that terminal. Zero or 2+ matches -> **do not guess**, **do not list which matched**, **do not grow a common prefix**. After every Tab, **always** refresh the status strip (never leave a stale line). Unique-complete then immediately teach the **new** hole (see Status after Tab).
4. **Whole-line unique-expand when caret is at end.** If and only if the first token is an exact verb or a unique verb prefix, Tab unique-expands every token. If the first token is not a unique verb (`xyz`, `address=$D020`), print verb help and **do not** expand later tokens.
5. **Open values are never completed.** Ids, limits, hex, ranges, opcode lists: Tab only prints syntax help.
6. **UI strings are ASCII-only.** Help uses `|` and `->`, never Unicode arrows or ellipses. Fit `FRONTEND_FR_STATUS_MAX` (192); truncate with ASCII `...` if needed.
7. **Find keys/values stay the shared tables.** `runtime_history_find_option_keys()` / `runtime_history_find_access_names()`. Do not hardcode a parallel find-key list. Verb names, `read`/`next` named keys, and help strings live in this guide’s tables. Find-key **help** is built from the shared key table. Access **help** stays the short catalog line (not the full name dump).
8. **Duplicate named keys: last wins** on Enter (same as shared find parser / parent PR 1).

v1 help is **Tab-driven** (status updates on Tab). Live-per-keystroke hints are optional later; same walker.

v1 caret model is **end of line**. Completing a hole in the middle of the line is out of scope.
...
Value syntax for `find-kv` is exactly the Find option grammar in
[`forensics-ui.md`](forensics-ui.md) (shared `runtime_history_parse_find_options`).
`id` is a retained record id: decimal or `$` / `0x` hex, `>= 1`.

`find` with no options is a valid command (parser defaults: `direction=backward`,
`limit=64`). `next` with no `limit=` uses 64. `read` without `id` is not
submittable. `info` takes no arguments; extra tokens are `bad-args` on Enter.

**`read` argument order (Enter):** named keys and `id` may appear in **any
order**. Exactly one `id` is required. `read before=2 42` and `read 42 before=2`
both submit. A second bare token is `bad-args`. Tab may complete `read bef` ->
`read before=` before an id exists; Enter still needs the id.

Duplicate named keys: **last wins**. Unknown keys / bad values: `bad-args` on Enter.
...
Named-key unique-complete uses **all legal keys for that verb** (duplicates allowed on the line; last wins on Enter). Empty-slot **help** omits keys already used, to stay short. Rewrite to **canonical table spelling** (`ADD` -> `address=`).
...
6. Rebuild the line with single spaces between tokens. Preserve a trailing space iff the input had trailing whitespace **or** the last rewrite was a verb complete. After `key=` with empty value, the line ends at `=`. Example: `f add=$D020`<Tab> -> `find address=$D020` (verb completed **and** later tokens unique-expanded).
...
8. If the line changed, set `query_rewrite_pending` (caret-to-end). `forensics_view_autocomplete` returns true **only** when the line rewrote; status may change on a false return.
...
3. Last token is complete (finished kv, finished id, exact verb with no trailing space):
   ...
   - Enter-valid and unused optional named keys remain -> remaining-keys help (unused find keys, or unused `read` keys). Example: `find add=$D020 acc=re`<Tab> -> `find address=$D020 access=read` + unused find-key help.
...
| `address=$D020` | (unchanged) | verb help (not implicit FIND; no later-token expand) |
| `f add=$D020` | `find address=$D020` | unused find-key help |
| `find add` | `find address=` | `pc/address: u16 or lo-hi ($hex ok)` |
| `find a` | `find a` | find-key help (`address` and `access`) |
| `find add=$D020 acc=re` | `find address=$D020 access=read` | unused find-key help (`re` -> `read` unique; `r` is not: `read` and `rmw-dummy-write`) |
| `find a=$D020 acc=re` | `find a=$D020 access=read` | unused find-key help (`a` not unique) |
| `find add=$D020 acc=r` | `find address=$D020 access=r` | access-value help (`r` ambiguous) |
...
| `address=$D020` + Enter | (no dispatch) | verb help (same string as Tab) |
...
Illustrative addresses use `$D020` (C64 VIC register) instead of Apple softswitch examples from a2m; grammar is unchanged.
