...
**Related:** `perf-baseline-turbo2.md` (numbers to beat),  
`perf-handoff-2026-07.md` (**session hand-off / next-agent start here**),  
`disk-iec1541.md` (soft power), `runtime-control.md` (turbo 1/2/3),  
`architecture.md` (threads).
...
| 2026-07-24 | Session hand-off: `perf-handoff-2026-07.md` (plateau ~16.5/22.4 MHz on/off, ~2.1× from ~8; structural next; diminishing micro-opts). |
| 2026-07-25 | Paint re-arch (local stack): double-buffered live frames + skip per-frame ARGB clear + solid-span flush flags. Pure paint-on holds ~**16.5–16.7 MHz** on M2. True 8-bit index FB attempted then backed out (ARGB-in-uint32 slots and reverse-lookup paths regressed; wire format / snapshot care needed for a second pass). |
| 2026-07-25 | Hazard strip: fuse instruction micro drain + free-pin micro_hot. ~**16.8 / 22.8 MHz** paint-on/off. Skipping BA checks under “no-BA line” without post-begin pin recheck is incorrect (breaks BA-stall tests); only elide access-kind when AEC+RDY high. |
