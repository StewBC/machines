...
- SID improvement Phase 10 (C64SID_IMP_10): `SID_HFROLL_COEFF` tightened from 0.895 (~16.5 kHz) to 0.940 (~9.4 kHz) in `sid_condition_output()`, modeling the combined 6581 chip-plus-board output-path rolloff rather than only the chip's internal output pin. Output gain retuned from 19% to 20% to restore RMS alignment after the tighter filter reduces overall output level. No struct or API changes; test comments updated to remove stale "16.5 kHz" references; all 27 tests pass. `el_cartero` Phase 10 metrics against `x64sc-20s.mp3`: score **1.2838** (improved from Phase 9 1.3534), correlation 0.7372, RMS 0.0674 vs VICE aligned 0.0670, spectral-band MAE 1.4512 dB (was 1.7828 dB). 16–22 kHz excess reduced from +9.92 dB to +6.34 dB (c64m −32.75 dB vs VICE −39.09 dB). 4–8 kHz band slightly darker (c64m −19.74 dB vs VICE −18.34 dB, −1.40 dB vs prior −0.55 dB). Results stable across repeated runs (0-block lag variation). Correlation slight regression from 0.7430 to 0.7372 is within normal run-to-run variation (music-position alignment noise) and not attributable to the coefficient change.
