...
- Control port Phases 1 through 6 are implemented as an opt-in localhost-only service with main-loop-owned runtime dispatch, execution/state commands, binary frame/memory/debug-memory responses, input injection, paste payloads, file/disk commands, breakpoint management, and wait commands. See `docs/status/CONTROL.md`.
