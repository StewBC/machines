...
- Memory/disassembly view source mode: Map (CPU-visible address space), ROM (physical ROM bytes regardless of mapping), RAM (raw RAM regardless of mapping). Mode is per-view and independent. Right-click contextual popup selects mode; footer button cycles Map→ROM→RAM; Opt+M keyboard shortcut cycles from the active view. ROM mode shows an amber border inside the content area; RAM mode shows a blue border; Map mode has no border.
