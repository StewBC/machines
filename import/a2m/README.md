...
```NOTE: This emulator is currently undergoing major upgrades.  I am uploading to GitHub as that's also my backup policy ;)  Be aware though, the code here is in flux.  I try to make sure it compiles everywhere and works, but as I go, pieces get upgraded and the user manual is not up to date.   For example, I just completely redid the memory view. Ther's no more CTRL+g to "go to" an address, instead it's CTRL+a to edit the address.  There many changes like that.  The memory view will listen to the layout changes, but the disassembler view is still the old view and will not react to layout changes.  When I have upgraded everything and am happy with where I am trying to go (I have a big to-do list), I will make a 2.0 release.  So, even though what's checked in is a ][+ or a //e Enhanced, what's written is applicable to the Release in Releases which is a ][+ emulator.  
```
