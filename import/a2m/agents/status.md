...
**Active:** **TimeMachine** ([`timemachine.md`](timemachine.md) — TM0–TM6; V1 = TM0–TM4). **TM0 landed:** opt-in `[debug] timemachine` / `--timemachine` (default off) arms HST1 + frame ring; `timemachine_memory_mb` plumbed (consumed in TM2). Next: TM1 query engine. F7 Inspector is **retired and removed** — lessons in [`inspector.md`](inspector.md), code at tag `archive/f7-inspector`.  
...
| Area | Evidence |
|------|----------|
| Build / ctest | **56 green** (`testing.md`; includes sessions + state-changed, file-codec, assembler MLI launch, TM0) |
