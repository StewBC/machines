...
There is no `$C800` expansion firmware in tree. When SP `$C800` is latched,
PC traps at `$C800` / `$C89B` / `$C9AA` dispatch **STATUS / READ_BLOCK /
WRITE_BLOCK** only (`sp_host_trap`). Other commands return `$27`. That trap is
not a 6502 opcode fetch.
