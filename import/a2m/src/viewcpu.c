...
void viewcpu_show(APPLE2* m)  {
    ...
    if(nk_begin(ctx, "CPU", nk_rect(m->viewport->target_rect.w, 0, w, 89),
        NK_WINDOW_NO_SCROLLBAR | NK_WINDOW_TITLE | NK_WINDOW_BORDER))
    {
        if(m->stopped) {
            ...
            if(NK_EDIT_COMMITED & nk_edit_string(ctx, NK_EDIT_SELECTABLE | NK_EDIT_CLIPBOARD | NK_EDIT_SIG_ENTER, vcpu->registers[0], &vcpu->register_lengths[0], 5, nk_filter_hex)) {
            ...
            nk_layout_row_push(ctx, 0.15f);
            if(NK_EDIT_COMMITED & nk_edit_string(ctx, NK_EDIT_SELECTABLE | NK_EDIT_CLIPBOARD | NK_EDIT_SIG_ENTER, vcpu->registers[1], &vcpu->register_lengths[1], 5, nk_filter_hex)) {
            ...
            if(NK_EDIT_COMMITED & nk_edit_string(ctx, NK_EDIT_SELECTABLE | NK_EDIT_CLIPBOARD | NK_EDIT_SIG_ENTER, vcpu->registers[2], &vcpu->register_lengths[2], 3, nk_filter_hex)) {
            ...
            nk_layout_row_push(ctx, 0.10f);
            if(NK_EDIT_COMMITED & nk_edit_string(ctx, NK_EDIT_SELECTABLE | NK_EDIT_CLIPBOARD | NK_EDIT_SIG_ENTER, vcpu->registers[3], &vcpu->register_lengths[3], 3, nk_filter_hex)) {
            ...
            nk_layout_row_push(ctx, 0.10f);
            if(NK_EDIT_COMMITED & nk_edit_string(ctx, NK_EDIT_SELECTABLE | NK_EDIT_CLIPBOARD | NK_EDIT_SIG_ENTER, vcpu->registers[4], &vcpu->register_lengths[4], 3, nk_filter_hex)) {
            ...
            for(int i=0; i < 8; i++) {
                ...
                if(NK_EDIT_COMMITED & nk_edit_string(ctx, NK_EDIT_SELECTABLE | NK_EDIT_CLIPBOARD | NK_EDIT_SIG_ENTER, vcpu->flags[i], &vcpu->flag_lengths[i], 2, nk_filter_binary)) {
            ...
            nk_labelf(ctx, NK_TEXT_ALIGN_LEFT | NK_TEXT_ALIGN_MIDDLE, 
                "PC %04X   SP %04X   A %02X   X %02X   Y %02X",
                m->cpu.pc, m->cpu.sp, m->cpu.A, m->cpu.X, m->cpu.Y);
            nk_labelf(ctx, NK_TEXT_ALIGN_LEFT | NK_TEXT_ALIGN_MIDDLE, 
