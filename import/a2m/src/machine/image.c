...
double image_cycles_per_byte(DISKII_IMAGE *image) {
    ...
    switch(image->kind) {
        case IMG_NIB: {
                IMAGE_NIB *nib = (IMAGE_NIB *)image->image_specifics;
                if(nib->track_size) {
                    return (APPLE2_CPU_FREQUENCY_HZ * 60.0) / (300.0 * (double)nib->track_size);
                }
                return 32.0;
            }
            break;
...
static uint64_t image_advance_bytes(apple2_t *m, DISKII_DRIVE *d, DISKII_IMAGE *image) {
    double delta = (double)m->cpu.cpu.cycles - d->q6_last_read_cycles;
...
void image_advance_position(apple2_t *m, DISKII_DRIVE *d) {
...
int image_load_dsk(apple2_t *m, DISKII_IMAGE *image, const char *ext) {
    UNUSED(m);
...
int image_load_nib(apple2_t *m, DISKII_IMAGE *image) {
...
int image_load_woz(apple2_t *m, DISKII_IMAGE *image) {
    UNUSED(m);
