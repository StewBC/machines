...
static void mockingboard_apply_via_to_ay(apple2_t *m, MOCKINGBOARD *mb, int slot,
    ...
    if((via->orb & 0x04) == 0) {
        ...
        ay38910_reset(ay, APPLE2_CPU_FREQUENCY_HZ, APPLE2_CPU_FREQUENCY_HZ);
        return;
    }
...
static void mockingboard_bind_via_context(apple2_t *m, MOCKINGBOARD *mb, int slot, uint8_t via_index) {
...
uint8_t mockingboard_read_via_port_a(const apple2_t *m, uint8_t slot, uint8_t pair_index) {
...
uint8_t mockingboard_irq_pending(apple2_t *m) {
    uint8_t slot = m->mb_slot;

    if(!slot || m->slot_type[slot] != SLOT_TYPE_MOCKINGBOARD) {
        return 0;
    }
...
void mockingboard_reset(MOCKINGBOARD *mb, int full) {
    ...
    ay38910_reset(&mb->ay[0], APPLE2_CPU_FREQUENCY_HZ, APPLE2_CPU_FREQUENCY_HZ);
    ay38910_reset(&mb->ay[1], APPLE2_CPU_FREQUENCY_HZ, APPLE2_CPU_FREQUENCY_HZ);
    if(full) {
...
uint8_t mockingboard_read_cn(apple2_t *m, MOCKINGBOARD *mb, int slot, uint16_t address, uint8_t slot_offset) {
    uint8_t via_index;
    uint8_t via_reg;
...
void mockingboard_write_cn(apple2_t *m, MOCKINGBOARD *mb, int slot, uint16_t address, uint8_t slot_offset, uint8_t value) {
    uint8_t via_index;
    uint8_t via_reg;
...
uint8_t mockingboard_read(apple2_t *m, MOCKINGBOARD *mb, int slot, uint16_t address, uint8_t reg) {
    // Compatibility alias:
...
void mockingboard_write(apple2_t *m, MOCKINGBOARD *mb, int slot, uint16_t address, uint8_t reg, uint8_t value) {
    // Compatibility alias:
    // C0n0-C0n7 -> VIA #0 register window, C0n8-C0nF -> VIA #1 register window.
    uint8_t via_index = (reg >> 3) & 0x01;
    uint8_t via_reg = reg & 0x07;
