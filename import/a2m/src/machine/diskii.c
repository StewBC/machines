#include "apple2.h"
#include "diskii.h"
#include "image.h"
#include "a2_status.h"
#include "dynarray.h"
#include "util_file.h"

#include <math.h>
#include <stdlib.h>
#include <string.h>
#if defined(_WIN32)
#define strcasecmp _stricmp
#else
#include <strings.h>
#endif
#include <time.h>



#define ms_to_cycles(x) (((x) * (uint64_t)APPLE2_CPU_FREQUENCY_HZ) / 1000)
#define us_to_cycles(x) (((x) * (uint64_t)APPLE2_CPU_FREQUENCY_HZ) / 1000000)
#define count_phases(x) ((x & 1) + ((x >> 1) & 1) + ((x >> 2) & 1) + ((x >> 3) & 1))
#define ROL4(x)         (((x << 1) | (x >> 3)) & 0xF)
#define ROR4(x)         (((x >> 1) | (x << 3)) & 0xF)

#define DISKII_NORMAL_RPM       300
#define DISKII_READABLE_RPM     200
#define DISKII_SPINUP_TIME      ms_to_cycles(400)
#define DISKII_SPINDOWN_TIME    ms_to_cycles(1000)
#define DISKII_MOTOR_OFF_DELAY  ms_to_cycles(1000)
#define DISKII_SPINUP_RATE      (double)DISKII_NORMAL_RPM / (double)DISKII_SPINUP_TIME    // RPM per cycle
#define DISKII_SPINDOWN_RATE    (double)DISKII_NORMAL_RPM / (double)DISKII_SPINDOWN_TIME

static inline double clamp(double v, double lo, double hi) {
    return v < lo ? lo : v > hi ? hi : v;
}

// Only gates readability during spin-up/down
static inline double rpm_now(uint64_t now, DISKII_DRIVE *d) {
    double dt = now - d->motor_event_cycles;
    double rate = DISKII_SPINUP_RATE;
    if(!d->motor_on) {
        if(dt < d->motor_off_delay_cycles) {
            return clamp(d->motor_rpm, 0.0, DISKII_NORMAL_RPM);
        }
        dt -= d->motor_off_delay_cycles;
        rate = -DISKII_SPINDOWN_RATE;
    }
    double rpm  = d->motor_rpm + rate * dt;
    return clamp(rpm, 0.0, DISKII_NORMAL_RPM);
}

static inline void diskii_timer_update(uint64_t now, uint64_t *anchor, uint64_t *timer) {
    uint64_t delta = now - *anchor;
    *anchor = now;
    *timer = *timer > delta ? *timer - delta : 0;
}

static inline double diskii_readable_rpm(DISKII_IMAGE *image) {
    return image && image->kind == IMG_WOZ ? 1.0 : DISKII_READABLE_RPM;
}

static uint8_t diskii_register_read(apple2_t *m, DISKII_DRIVE *d) {
    if(!d->active_image || !d->active_image->image_specifics) {
        return 0x7f;
    }

    if(d->q7) {
        return d->q6 ? 0x7f : 0x80;
    }

    if(d->q6) {
        return d->sensor_protect ? 0x80 : 0x00;
    }

    diskii_timer_update(m->cpu.cpu.cycles, &d->head_event_cycles, &d->head_settle_cycles);
    if((double)m->cpu.cpu.cycles - d->q6_last_read_cycles < image_cycles_per_byte(d->active_image) ||
            rpm_now(m->cpu.cpu.cycles, d) < diskii_readable_rpm(d->active_image) || d->head_settle_cycles) {
        return 0x7f;
    }

    uint8_t value = image_get_byte(m, d);
    if(value & 0x80) {
        d->read_latch = value;
    }
    return value;
}

static void diskii_write_latch_to_disk(apple2_t *m, DISKII_DRIVE *d) {
    if(d->sensor_protect || !d->write_latch_valid) {
        return;
    }

    (void)image_put_byte(m, d, d->write_latch);
}

void diskii_controller_init(DISKII_CONTROLLER *controller) {
    ARRAY_INIT(&controller->diskii_drive[0].images, DISKII_IMAGE);
    ARRAY_INIT(&controller->diskii_drive[1].images, DISKII_IMAGE);
}

void diskii_drive_select(apple2_t *m, const int slot, int soft_switch) {
    DISKII_CONTROLLER *dc = &m->diskii_controller[slot];
    DISKII_DRIVE *d = &dc->diskii_drive[dc->active];
    if(d->write_active) {
        image_finish_write(m, d);
        d->write_active = 0;
    }
    m->diskii_controller[slot].active = soft_switch & 1;
}

void diskii_motor(apple2_t *m, const int slot, int soft_switch) {
    DISKII_DRIVE *d = &m->diskii_controller[slot].diskii_drive[m->diskii_controller[slot].active];
    uint64_t now = m->cpu.cpu.cycles;
    double current_rpm = rpm_now(now, d);
    d->motor_rpm = current_rpm;
    d->motor_event_cycles = now;
    d->motor_on = soft_switch & 1;
    d->motor_off_delay_cycles = d->motor_on ? 0 : DISKII_MOTOR_OFF_DELAY;
    d->q6_last_read_cycles = (double)now;
    if(!d->motor_on && d->write_active) {
        image_finish_write(m, d);
        d->write_active = 0;
    }
}

int diskii_eject(apple2_t *m, const int slot, const int device, int mount_next) {
    DISKII_CONTROLLER *dc;
    if(m == NULL || slot < 1 || slot > 7 || device < 0 || device > 1) {
        return A2_ERR;
    }
    dc = &m->diskii_controller[slot];

    DISKII_DRIVE *dd = &dc->diskii_drive[device];
    if(!dd->active_image) {
        return A2_OK;
    }

    if(dd->write_active) {
        if(A2_OK != image_finish_write(m, dd)) {
            return A2_ERR;
        }
        dd->write_active = 0;
    }
    if(image_is_dirty(dd->active_image) && A2_OK != image_save(dd->active_image)) {
        return A2_ERR;
    }
    image_shutdown(dd->active_image);
    array_remove(&dd->images, dd->active_image);
    dd->active_image = NULL;
    int index = dd->image_index;
    size_t items = dd->images.items;
    if(mount_next && items > 0) {
        diskii_mount_image(m, slot, device, index >= items ? 0 : index);
    } else {
        dd->image_index = -1;
    }
    return A2_OK;
}

int diskii_mount(apple2_t *m, const int slot, const int device, const char *file_name) {
    DISKII_CONTROLLER *dc = &m->diskii_controller[slot];
    if(slot < 1 || slot > 7) {
        return A2_ERR;
    }
    DISKII_DRIVE *dd = &dc->diskii_drive[device];
    int previous_index = dd->image_index;
    DISKII_IMAGE new_disk_image;
    memset(&new_disk_image, 0, sizeof(DISKII_IMAGE));
    DISKII_IMAGE *image = &new_disk_image;
    // DISKII_IMAGE *image = &dd->image;
    UTIL_FILE *file = &image->file;
    if(A2_OK != util_file_load(file, file_name, "rb+")) {
        return A2_ERR;
    }

    const char *ext = strrchr(file_name, '.');
    int load;
    if(file->file_size == 143360 || (ext && (0 == strcasecmp(ext, ".dsk") || 0 == strcasecmp(ext, ".do") || 0 == strcasecmp(ext, ".po")))) {
        // DSK file
        load = image_load_dsk(m, image, ext);
    } else if(file->file_size > 79 && 0 == strncmp(file->file_data, "WOZ", 3)) {
        // WOZ file
        load = image_load_woz(m, image);
    } else {
        if(file->file_size == 6656 * 35 || file->file_size == 6384 * 35 || (ext && 0 == strcasecmp(ext, ".nib"))) {
            // NIB file
            load = image_load_nib(m, image);
        } else {
            return A2_ERR;
        }
    }
    if(load == A2_OK && dd->active_image != NULL) {
        if(diskii_save(m, slot, device) != A2_OK) {
            image_shutdown(&new_disk_image);
            return A2_ERR;
        }
        /* ARRAY_ADD may relocate the backing store and invalidate this pointer. */
        dd->active_image = NULL;
    }
    if(load != A2_OK || A2_ERR == ARRAY_ADD(&dd->images, new_disk_image)) {
        image_shutdown(&new_disk_image);
        if(previous_index >= 0 && (size_t)previous_index < dd->images.items) {
            dd->active_image = ARRAY_GET(&dd->images, DISKII_IMAGE, previous_index);
        }
        return A2_ERR;
    }
    dd->image_index = dd->images.items - 1;
    return diskii_mount_image(m, slot, device, dd->image_index);
}

uint8_t diskii_mount_image(apple2_t *m, const int slot, const int device, const int index) {
    DISKII_CONTROLLER *dc = &m->diskii_controller[slot];
    if(slot < 1 || slot > 7 || device < 0 || device > 1) {
        return A2_ERR;
    }
    DISKII_DRIVE *dd = &dc->diskii_drive[device];
    /* 0-based index; valid range is [0, items). */
    if(!dd->images.items || index < 0 || (size_t)index >= dd->images.items) {
        dd->image_index = -1;
        dd->active_image = NULL;
        return A2_ERR;
    }
    /* A queue transition is a persistence boundary. Keep the queued image in
       memory for fast swaps, but commit any completed writes before changing
       which image is active. */
    if(dd->active_image != NULL && dd->active_image != ARRAY_GET(&dd->images, DISKII_IMAGE, index)) {
        if(diskii_save(m, slot, device) != A2_OK) {
            return A2_ERR;
        }
    }
    dd->active_image = ARRAY_GET(&dd->images, DISKII_IMAGE, index);
    apple2_install_diskii_rom(m, slot, (int)dd->active_image->disk_encoding);
    dd->quarter_track_pos = (int16_t)(apple2_rand_u32(m) % DISKII_QUATERTRACKS);
    dd->image_index = index;
    dd->write_latch_valid = 0;
    dd->read_latch = 0x7f;
    dd->write_active = 0;
    dd->write_track = 0;
    dd->write_start_pos = 0;
    dd->write_byte_count = 0;
    dd->q6 = 0;
    dd->q7 = 0;
    return A2_OK;
}

int diskii_swap_image(
    apple2_t *m,
    const int slot,
    const int device,
    int32_t param,
    int relative)
{
    DISKII_DRIVE *dd;
    int count;
    int current;
    int new_index;

    if(m == NULL || slot < 1 || slot > 7 || device < 0 || device > 1) {
        return A2_ERR;
    }
    if(!m->diskii_present[slot]) {
        return A2_ERR;
    }
    dd = &m->diskii_controller[slot].diskii_drive[device];
    count = (int)dd->images.items;
    if(count <= 0) {
        return A2_ERR;
    }
    /* Bare SWAP (param 0) means next image. */
    if(param == 0) {
        param = 1;
        relative = 1;
    }
    current = dd->image_index;
    if(current < 0 || current >= count) {
        current = 0;
    }
    if(relative) {
        new_index = ((current + (int)param) % count + count) % count;
    } else {
        /* Absolute 1-based index, wrap. */
        new_index = (((int)param - 1) % count + count) % count;
    }
    return diskii_mount_image(m, slot, device, new_index) == A2_OK ? A2_OK : A2_ERR;
}

int diskii_save(apple2_t *m, const int slot, const int device) {
    DISKII_CONTROLLER *dc;
    if(m == NULL || slot < 1 || slot > 7 || device < 0 || device > 1) {
        return A2_ERR;
    }
    dc = &m->diskii_controller[slot];

    DISKII_DRIVE *dd = &dc->diskii_drive[device];
    if(!dd->active_image) {
        return A2_OK;
    }
    if(dd->write_active) {
        if(A2_OK != image_finish_write(m, dd)) {
            return A2_ERR;
        }
        dd->write_active = 0;
    }
    return image_save(dd->active_image);
}

int diskii_flush_all(apple2_t *m) {
    int slot;
    int device;

    if(m == NULL) {
        return A2_ERR;
    }
    for(slot = 1; slot <= 7; ++slot) {
        if(!m->diskii_present[slot]) {
            continue;
        }
        for(device = 0; device < 2; ++device) {
            DISKII_DRIVE *dd = &m->diskii_controller[slot].diskii_drive[device];
            size_t i;
            if(dd->write_active) {
                if(image_finish_write(m, dd) != A2_OK) {
                    return A2_ERR;
                }
                dd->write_active = 0;
            }
            for(i = 0; i < dd->images.items; ++i) {
                DISKII_IMAGE *image = ARRAY_GET(&dd->images, DISKII_IMAGE, i);
                if(image != NULL && image_is_dirty(image) && image_save(image) != A2_OK) {
                    return A2_ERR;
                }
            }
        }
    }
    return A2_OK;
}

uint8_t diskii_q6_access(apple2_t *m, int slot, uint8_t on_off, int write_access) {
    DISKII_DRIVE *d = &m->diskii_controller[slot].diskii_drive[m->diskii_controller[slot].active];
    uint8_t old_q6 = d->q6;
    d->q6 = on_off;
    if(!write_access && d->q6) {
        image_reset_latch(d->active_image);
        d->read_latch = 0x7f;
    }
    if(d->q7 && old_q6 && !d->q6) {
        diskii_write_latch_to_disk(m, d);
    }
    return diskii_register_read(m, d);
}

uint8_t diskii_q7_access(apple2_t *m, int slot, uint8_t on_off) {
    DISKII_DRIVE *d = &m->diskii_controller[slot].diskii_drive[m->diskii_controller[slot].active];
    if(d->q7 && !on_off) {
        image_finish_write(m, d);
        d->write_active = 0;
    }
    d->q7 = on_off;
    return diskii_register_read(m, d);
}

uint8_t diskii_latch_read(apple2_t *m, int slot) {
    DISKII_DRIVE *d = &m->diskii_controller[slot].diskii_drive[m->diskii_controller[slot].active];
    return d->read_latch;
}

void diskii_write_access(apple2_t *m, int slot, uint8_t value) {
    DISKII_DRIVE *d = &m->diskii_controller[slot].diskii_drive[m->diskii_controller[slot].active];
    if(!d->active_image || !d->active_image->image_specifics || !d->q7 || !d->q6) {
        return;
    }

    d->write_latch = value;
    d->write_latch_valid = 1;
}

void diskii_reset(apple2_t *m) {
    for(int slot = 1; slot <= 7; slot++) {
        if(m->diskii_present[slot]) {
            diskii_drive_select(m, slot, IWM_SEL_DRIVE_2);
            diskii_motor(m, slot, IWM_MOTOR_OFF);
            diskii_drive_select(m, slot, IWM_SEL_DRIVE_1);
            diskii_motor(m, slot, IWM_MOTOR_OFF);
        }
    }
}

void diskii_shutdown(apple2_t *m) {
    (void)diskii_flush_all(m);
    for(int slot = 1; slot <= 7; slot++) {
        if(m->diskii_present[slot]) {
            DISKII_DRIVE *d0 = &m->diskii_controller[slot].diskii_drive[0];
            if(d0->write_active) {
                image_finish_write(m, d0);
                d0->write_active = 0;
            }
            for(int i = 0; i < d0->images.items; i++) {
                image_shutdown(ARRAY_GET(&d0->images, DISKII_IMAGE, i));
            }
            m->diskii_controller[slot].diskii_drive[0].active_image = NULL;
            DISKII_DRIVE *d1 = &m->diskii_controller[slot].diskii_drive[1];
            if(d1->write_active) {
                image_finish_write(m, d1);
                d1->write_active = 0;
            }
            for(int i = 0; i < d1->images.items; i++) {
                image_shutdown(ARRAY_GET(&d1->images, DISKII_IMAGE, i));
            }
            m->diskii_controller[slot].diskii_drive[1].active_image = NULL;
            array_free(&d0->images);
            array_free(&d1->images);
        }
    }
}

void diskii_step_head(apple2_t *m, int slot, int soft_switch) {
    DISKII_DRIVE *d = &m->diskii_controller[slot].diskii_drive[m->diskii_controller[slot].active];

    diskii_timer_update(m->cpu.cpu.cycles, &d->head_event_cycles, &d->head_settle_cycles);

    uint8_t phase_bit = (1u << ((soft_switch >> 1) & 3)); // PH0..PH3
    uint8_t on = (soft_switch & 1);

    uint8_t prev_on = d->last_on_phase_mask;             // last non-zero on state
    uint8_t curr = on ? (d->phase_mask | phase_bit) : (d->phase_mask & ~phase_bit);

    if(curr == d->phase_mask) {
        return;                                          // no change
    }

    int prev_n = count_phases(d->phase_mask);
    int n = count_phases(curr);
    if(n == 0 || n > 2) {                                // invalid holding pattern
        d->phase_mask  = curr;
        return;
    }

    // Determine direction relative to previous on mask.
    int step_qtr = (n == 1 && prev_n != 2) ? 2 : 1;       // 2-phase transitions move a single quarter-track
    if(curr & ROL4(prev_on)) {
        // forward
    } else if(curr & ROR4(prev_on)) {
        step_qtr = -step_qtr;                            // reverse
    } else {
        d->phase_mask  = curr;
        if(curr) {
            d->last_on_phase_mask = curr;
        }
        return;
    }

    d->head_settle_cycles = ms_to_cycles(3);
    d->quarter_track_pos += step_qtr;
    if(n == 1 && prev_n != 2) {
        d->quarter_track_pos &= ~1;                       // snap single-phase to even qtracks
    }
    if(d->quarter_track_pos < 0) {
        d->quarter_track_pos = 0;
    } else if(d->quarter_track_pos >= DISKII_QUATERTRACKS) {
        d->quarter_track_pos = DISKII_QUATERTRACKS - 1;
    }

    if(d->active_image) {
        image_advance_position(m, d);
        image_head_position(d->active_image, d->quarter_track_pos);
    }

    d->phase_mask = curr;
    if(curr) {
        d->last_on_phase_mask = curr;                    // preserve across all off
    }
}
