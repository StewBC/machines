...
"The machine configuration dialog allows common emulator settings to be edited without hand-editing the INI file. It has three tabs: <#QU>Machine<#NM>, <#QU>Emulator<#NM>, and <#QU>Assembler<#NM>.\n"
"\n"
"On the <#QU>Machine<#NM> tab, select the model (<#QU>Apple ][+<#NM> or <#QU>Apple //e Enhanced<#NM>) and the device installed in slots 1-7. On the ][+ model, Slot 3 also allows a Franklin Ace Display card.\n"
"\n"
"On the <#QU>Emulator<#NM> tab, configure the UI (<#QU>Gui<#NM> or <#QU>Text<#NM>), the scroll wheel speed, Disk LEDs, and symbol files (a comma-separated list). The symbol files row includes a <#QU>[Browse]<#NM> button. There is also a <#QU>Remember INI settings<#NM> / <#QU>Save settings on Quit<#NM> checkbox, and a <#QU>Turbo Speeds<#NM> field for comma-separated multipliers such as <#QU>1, 8, max<#NM>.\n"
"\n"
"On the <#QU>Assembler<#NM> tab, select the assembler source file (with a <#QU>[Browse]<#NM> button), pick the destination bank, and configure <#QU>Reset Stack<#NM>, <#QU>Auto Run<#NM>, and the <#QU>Auto Run Address<#NM>.\n"
"\n"
"At the bottom is the <#QU>INI file name<#NM> field with its own <#QU>[Browse]<#NM> button, plus <#QU>[OK]<#NM> and <#QU>[Cancel]<#NM> buttons. <#QU>[OK]<#NM> applies the changes; <#QU>[Cancel]<#NM> closes the dialog without applying them.\n"
"\n"
