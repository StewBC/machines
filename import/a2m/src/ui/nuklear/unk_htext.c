...
".segdef  \"n\",s[,e]   Define a named segment n, starting at s, and emit_byte (default) or noemit\n"
...
".include \"mminer.asm\"\n"
"<#NM>\n"
"Named segments are created by <#QU>.segdef<#NM>. When a segment is activated using, for example, <#QU>.segment \"ZEROPAGE\"<#NM>, the code emit_byte location is set to <#QU>$50<#NM>. Each instruction that would normally emit_byte code advances the location counter, but because <#QU>ZEROPAGE<#NM> is marked <#QU>noemit<#NM>, no bytes are written.\n"
"\n"
...
"Notes:\n"
"\n"
"* The assembler allows segments to overflow. For segments that are marked as <#QU>emit_byte<#NM>, the assembler will issue a warning when overflow occurs and will report the amount by which a segment size must be adjusted to allow segments to abut correctly.\n"
