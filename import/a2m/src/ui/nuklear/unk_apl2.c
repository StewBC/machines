...
mRGB palette_16[16] = {
    ...
    { 0x9D, 0x09, 0x66 },                                   // 01 Violet
    { 0x2A, 0x2A, 0xE5 },                                   // 02 Dark blue
    { 0xC7, 0x34, 0xFF },                                   // 03 Purple/Magenta
    { 0x00, 0x80, 0x00 },                                   // 04 Dark green
    { 0x80, 0x80, 0x80 },                                   // 05 Gray1
    { 0x0D, 0xA1, 0xFF },                                   // 06 Meduim blue
    { 0xAA, 0xAA, 0xFF },                                   // 07 Light Blue
    { 0x55, 0x55, 0x00 },                                   // 08 Brown
    { 0xF2, 0x5E, 0x00 },                                   // 09 Orange
    { 0xC0, 0xC0, 0xC0 },                                   // 10 Gray2
    { 0xFF, 0x89, 0xE5 },                                   // 11 Pink
    { 0x38, 0xCB, 0x00 },                                   // 12 Green
    { 0xD5, 0xD5, 0x1A },                                   // 13 Yellow
    { 0x62, 0xF6, 0x99 },                                   // 14 Aqua (Cyan)
    { 0xFF, 0xFF, 0xFF },                                   // 15 White
};

// Map the LORES palette for DHGR use
const int pattern_to_color[16] = {
    0,  // 0000 - Black
    8,  // 0001 - Brown (or Orange depending on phase)
    4,  // 0010 - Dark Green
    12, // 0011 - Green
    2,  // 0100 - Dark Blue
    10, // 0101 - Gray 2
...
static inline int rotate_right(int nibble, int phase){
    ...
    for(int pattern = 0; pattern < 32; pattern++) {
        for(int phase = 0; phase < 4; phase++) {
            ...
            if (force_white) {
                ...
                // Use 4 bits for color
                int nibble = (pattern >> 1) & 0xF; 
                int rotated = rotate_right(nibble, phase);
                color_idx = pattern_to_color[rotated];
            }
            dhgr_lut[pattern][phase] = SDL_MapRGB(format, palette_16[color_idx].r, palette_16[color_idx].g, palette_16[color_idx].b);
        }
    }
}
        ...
        if(mod & KMOD_CTRL) {
            ...
            if(e->key.keysym.scancode != SDL_SCANCODE_LCTRL) {
                switch(e->key.keysym.sym) {
                    ...
                    case SDLK_PAUSE:
                        // Reset the machine
                        rt_machine_reset(rt, mod & KMOD_ALT);
...
void unk_apl2_screen_dhgr(UNK *v, int start, int end) {
    ...
    for(int y = start; y < end; y++) {
        ...
        uint8_t row_560[565];
        int index = 2;
        row_560[0] = row_560[1] = row_560[562] = row_560[563] = row_560[564] = 0;
        ...
        if(!(v->monitor_type & MONITOR_MONO)) {
            for(int x = 0; x < 560; x++) {
                // A 5-bit window centered on x
                // row_560 must be padded so x-2 and x+2 are always safe
                uint8_t bits = (row_560[x] << 4) | (row_560[x+1] << 3) | (row_560[x+2] << 2) | (row_560[x+3] << 1) | row_560[x+4];
                *p++ = dhgr_lut[bits][(x + 3) & 3];
            }
        }
    }
}
