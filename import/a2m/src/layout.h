...
void layout_init(LAYOUT *l, float split_top_left_right, float split_top_bottom, float split_bottom_left_right, int apple2_px_w, int apple2_px_h);

//  Pixel-seeded init (use this): Seeds Apple II pixel size; derives A and B from parent size; C from mem_ratio
void layout_init_apple2(LAYOUT *l, struct nk_rect parent, int apple2_px_w, int apple2_px_h, float mem_ratio); //  0..1 for Mem within bottom
...
void layout_on_drag_split_A(LAYOUT *l, float dx_pixels, struct nk_rect parent, const LAYOUT_LIMITS *lim);
void layout_on_drag_split_B(LAYOUT *l, float dy_pixels, struct nk_rect parent, const LAYOUT_LIMITS *lim);
void layout_on_drag_split_C(LAYOUT *l, float dx_pixels, struct nk_rect parent, const LAYOUT_LIMITS *lim);

//  Apple II corner drag; proportional by default. If preserve_aspect=false, applies dx/dy independently (non-uniform).
void layout_on_drag_a2_corner(LAYOUT *l, float dx_pixels, float dy_pixels, int preserve_aspect, struct nk_rect parent, const LAYOUT_LIMITS *lim);
...
static inline void layout_clear_drag(LAYOUT *l) {
