...
uint8_t apple2_softswitch_read_callback(APPLE2 *m, uint16_t address) {
    if(address >= 0xC080 && address <= 0xC08F) {
        ...
        switch(address) {
            case KBD: 
                // $C000 is fed through viewapl2_process_event or viewapl2_feed_clipboard_key
                return m->RAM_MAIN[KBD];
            case KBDSTRB:
                if(m->clipboard_text) {
            ...
            case RDVBL:     //e
                if(m->model) {
                    ...
                    // and 65 * 262 = 17030
                    uint64_t delta = m->cpu.cycles - m->vbl_cycles;
                    uint64_t frames = delta / 17030;
                    m->vbl_cycles += frames * 17030;
                    return ((m->cpu.cycles - m->vbl_cycles) % 17030);
                }
                break;
            case RDTEXT: // e
                if(m->model) {
