...
static inline void aiyr_und(APPLE2 *m) {
    a(m);
...
static inline void aipxr_und(APPLE2 *m) {
    a(m);
    m->cpu.scratch_hi = m->cpu.address_hi + 1;
    uint8_t lo = m->cpu.address_lo;
    m->cpu.address_lo += m->cpu.X;
...
static inline void miyr_und(APPLE2 *m) {
    al_read_pc(m);
    sl_read_a16(m);
    ah_read_a16_sl2al(m);
    m->cpu.scratch_hi = m->cpu.address_hi + 1;
...
static inline void unstable_store_a16(APPLE2 *m, uint8_t value) {
    if(m->cpu.page_fault) {
...
static inline void ahx_a16(APPLE2 *m) {
    unstable_store_a16(m, m->cpu.A & m->cpu.X & m->cpu.scratch_hi);
}

static inline void anc_imm(APPLE2 *m) {
    m->cpu.scratch_lo = read_from_memory(m, m->cpu.pc);
    m->cpu.pc++;
...
static inline void alr_imm(APPLE2 *m) {
    m->cpu.scratch_lo = read_from_memory(m, m->cpu.pc);
    m->cpu.pc++;
    m->cpu.A &= m->cpu.scratch_lo;
...
static inline void arr_imm(APPLE2 *m) {
    m->cpu.scratch_lo = read_from_memory(m, m->cpu.pc);
    m->cpu.pc++;
...
static inline void axs_imm(APPLE2 *m) {
    m->cpu.scratch_lo = read_from_memory(m, m->cpu.pc);
    m->cpu.pc++;
...
static inline void dcp_a16(APPLE2 *m) {
    m->cpu.scratch_lo--;
    write_to_memory(m, m->cpu.address_16, m->cpu.scratch_lo);
    compare_bytes(m, m->cpu.A, m->cpu.scratch_lo);
    CYCLE(m);
...
static inline void isc_a16(APPLE2 *m) {
    m->cpu.scratch_lo++;
    write_to_memory(m, m->cpu.address_16, m->cpu.scratch_lo);
...
static inline void jam(APPLE2 *m) {
    read_pc(m);
...
static inline void las_a16(APPLE2 *m) {
...
static inline void lax_a16(APPLE2 *m) {
    m->cpu.scratch_lo = read_from_memory(m, m->cpu.address_16);
    m->cpu.A = m->cpu.scratch_lo;
    set_register_to_value(m, &m->cpu.X, m->cpu.scratch_lo);
    CYCLE(m);
...
static inline void lax_imm_und(APPLE2 *m) {
    m->cpu.scratch_lo = read_from_memory(m, m->cpu.pc);
    m->cpu.pc++;
...
static inline void rla_a16(APPLE2 *m) {
...
static inline void sax_a16(APPLE2 *m) {
    write_to_memory(m, m->cpu.address_16, m->cpu.A & m->cpu.X);
    CYCLE(m);
}

static inline void slo_a16(APPLE2 *m) {
    m->cpu.C = m->cpu.scratch_lo & 0x80 ? 1 : 0;
    m->cpu.scratch_lo <<= 1;
    write_to_memory(m, m->cpu.address_16, m->cpu.scratch_lo);
    set_register_to_value(m, &m->cpu.A, m->cpu.A | m->cpu.scratch_lo);
...
static inline void sre_a16(APPLE2 *m) {
    m->cpu.C = m->cpu.scratch_lo & 0x01 ? 1 : 0;
    m->cpu.scratch_lo >>= 1;
    write_to_memory(m, m->cpu.address_16, m->cpu.scratch_lo);
    set_register_to_value(m, &m->cpu.A, m->cpu.A ^ m->cpu.scratch_lo);
...
static inline void rra_a16(APPLE2 *m) {
    uint8_t c = m->cpu.scratch_lo & 0x01;
...
static inline void xaa_imm(APPLE2 *m) {
    m->cpu.scratch_lo = read_from_memory(m, m->cpu.pc);
    m->cpu.pc++;
