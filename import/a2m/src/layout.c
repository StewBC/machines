...
void layout_init(LAYOUT *l, float A, float B, float C, int a2w, int a2h) {
...
void layout_init_apple2(LAYOUT *l, struct nk_rect parent, int a2w, int a2h, float mem_ratio) {
...
void layout_compute(LAYOUT *l, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
    ...
    struct nk_rect *R[] = { &l->apple2, &l->cpu, &l->disasm, &l->mem, &l->misc };
    for(int i = 0; i < 5; ++i) {
        R[i]->x = roundf(R[i]->x);
        R[i]->y = roundf(R[i]->y);
        R[i]->w = roundf(R[i]->w);
...
void layout_on_window_resize(LAYOUT *l, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
    // Keep Apple II size on OS resize
    // Re-derive A and B from the remembered Apple II px size
    // float A = (float)l->apple2_px_w / parent.w;
    // float B = (float)l->apple2_px_h / parent.h;

    // // Clamp to structural minimums
    // float A_min = (float)lim->min_a2_w_px / parent.w;
    // float A_max = 1.0f - (float)lim->min_right_w_px / parent.w;
    ...
    // C (Mem/Misc ratio) stays as-is
}

// Splitter drags
void layout_on_drag_split_A(LAYOUT *l, float dx, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
...
void layout_on_drag_split_B(LAYOUT *l, float dy, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
...
void layout_on_drag_split_C(LAYOUT *l, float dx, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
...
void layout_on_drag_a2_corner(LAYOUT *l, float dx, float dy, int preserve_aspect, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
    ...
    layout_set_apple2_pixels(l, new_w, new_h, parent, lim);
}

// Dragging layout handler
int layout_handle_drag(LAYOUT *l, const struct nk_input *in, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
    ...
    if(l->drag_active && in->mouse.buttons[NK_BUTTON_LEFT].down) {
        ...
        switch(l->drag_active) {
            case LAYOUT_DRAG_SPLIT_A:
                layout_on_drag_split_A(l, dx, parent, lim);
                changed = 1;
                break;
            case LAYOUT_DRAG_SPLIT_B:
                layout_on_drag_split_B(l, dy, parent, lim);
                changed = 1;
                break;
            case LAYOUT_DRAG_SPLIT_C:
                layout_on_drag_split_C(l, dx, parent, lim);
                changed = 1;
                break;
            case LAYOUT_DRAG_APPLE2_CORNER:
                layout_on_drag_a2_corner(l, dx, dy, 1, parent, lim);
                changed = 1;
                break;
            default:
                break;
...
void layout_set_apple2_pixels(LAYOUT *l, int new_w, int new_h, struct nk_rect parent, const LAYOUT_LIMITS *lim) {
    l->apple2_px_w = new_w;
    l->apple2_px_h = new_h;
    // l->apple2_aspect = (l->apple2_px_h > 0) ? (float)l->apple2_px_w / (float)l->apple2_px_h : l->apple2_aspect;
