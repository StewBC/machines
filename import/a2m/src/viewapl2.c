...
void viewapl2_screen_lores(APPLE2 *m, int start, int end) {
    ...
    for (y = start; y < end; y++) {
        ...
        for (int x = 0; x < 40; x++) {
            ...
            for(r = 0; r < 4; r++) {
                pr[0] = pu; pr[1] = pu; pr[2] = pu; pr[3] = pu; pr[4] = pu; pr[5] = pu; pr[6] = pu;
                pr += surface->w;
            }

            for(r = 0; r < 4; r++) {
                pr[0] = pl; pr[1] = pl; pr[2] = pl; pr[3] = pl; pr[4] = pl; pr[5] = pl; pr[6] = pl;
                pr += surface->w;
            }
            p += 7;
        }
...
void viewapl2_screen_lores_mono(APPLE2 *m, int start, int end) {
    ...
    for (y = start; y < end; y++) {
        ...
        for (int x = 0; x < 40; x++) {
            ...
            uint8_t lower = (character >> 4) & 0X0F;
            uint32_t *pr = p;
            uint32_t pu = SDL_MapRGB(surface->format, lores_palette_mono[upper].r, lores_palette_mono[upper].g, lores_palette_mono[upper].b);
            uint32_t pl = SDL_MapRGB(surface->format, lores_palette_mono[lower].r, lores_palette_mono[lower].g, lores_palette_mono[lower].b);
...
void viewapl2_screen_hgr_mono(APPLE2 *m, int start, int end) {
    ...
    for (y = start; y < end; y++) {
        ...
        for (int x = 0; x < 40; x++) {
            ...
            p[0] = c[pixels & 1]; pixels >>= 1;
            p[1] = c[pixels & 1]; pixels >>= 1;
            p[2] = c[pixels & 1]; pixels >>= 1;
            p[3] = c[pixels & 1]; pixels >>= 1;
            p[4] = c[pixels & 1]; pixels >>= 1;
            p[5] = c[pixels & 1]; pixels >>= 1;
            p[6] = c[pixels & 1];
            p += 7;
        }
    }
...
int viewapl2_speaker_init() {
    // init the Audio with a desired spec
...
void viewapl2_speaker_update(APPLE2 *m) {
    ...
    if(--speaker->current_rate <= 0.0f) {
        viewapl2_speaker_play(speaker);
        speaker->current_rate += speaker->sample_rate;
    }
}
