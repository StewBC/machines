#!/usr/bin/env python3
"""A2M/12 control-port client for a2m.
...
GOTCHAS (Apple A2M/12):
  * Identity: hello -> name=a2m protocol=A2M/12
  * Forensic: get-state includes mode=live|forensic focus_cycle= start= start_arg1=.
    exit-forensic restores live NOW (no auto-resume). Mutating verbs fail with
    error read-only-forensic. Tape seek/step are not on the wire.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="a2m control client (A2M/12)")
