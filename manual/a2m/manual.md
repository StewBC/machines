...
**Symbol Files** / `symbol_files` (and `--symbols`). Soft-disabled sources are omitted
from this list, from Disassembly labels, and from control `find-symbol`.
...
`symbol_files` / Configure **Symbol Files** (and `--symbols`) also publish into
the same table. `find-symbol` resolves a label from the **enabled** published set
by exact name (Symbol Filter soft-disables omit that source):
...
If the name is not present the reply is `error not-found`. If no assembly or
symbol file has published symbols yet, the reply is `error not-ready`. In the GUI
a successful remote assembly or file load refreshes Disassembly and Symbol Lookup
just as the Assembler tab and Configure symbol reload do.
