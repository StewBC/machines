...
1. File → `samples/apple2/asm_mli_launch/mli_launch.s`
...
into `samples/apple2/hostfs/mminer`.

With `samples/apple2/hostfs` mounted as SmartPort `S7D0`, set `Auto-run at` to
`$3000` and enable `MLI launch`.

Then, with `samples/apple2/mminer/mminer-a2m.asm` selected in the assembly window,
pressing `Assemble` will assemble and launch one of the following:

- `samples/apple2/hostfs/mminer/mminer.system#FF2000`
- `samples/apple2/hostfs/mminer/mminer#064000`
