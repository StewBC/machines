...
static runtime *start_real_1541_runtime(runtime_client **out_client) {
    ...
    config.turbo_speeds[0] = 2; /* max: free-run skip-ahead for load */
...
static void test_real_1541_autorun_loads_first_program(void) {
    ...
    runtime_client *client;
    runtime_event event;
    const char *disk_path = C64M_SOURCE_DIR "/assets/c64/disks/GALENCIA.D64";
...
int main(void) {
    {
        ...
        snprintf(asset_path, sizeof(asset_path),
                 "%s/assets/c64/disks/GALENCIA.D64", C64M_SOURCE_DIR);
